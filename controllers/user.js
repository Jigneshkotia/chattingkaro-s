...
const logout = TryCatch(async (req, res) => {
  return res
    .status(200)
    .cookie("chat-ka-token", "", { ...cookieOptions, maxAge: 0 })
    .json({
      success: true,
...
const searchUser = TryCatch(async (req, res) => {
  ...
  const allUsersExeptMeAndMyFriends = await User.find({
    _id: { $nin: allUsersFromMyChats },
    name: { $regex: name, $options: "i" },
  });

  const users = allUsersExeptMeAndMyFriends.map(({ _id, name, avatar }) => ({
    _id,
    name,
    avatar: avatar.url,
  }));
...
const acceptFriendRequest = TryCatch(async (req, res,next) => {
  ...
  if (!request) await next(new ErrorHandler("Request Not Found", 404));
